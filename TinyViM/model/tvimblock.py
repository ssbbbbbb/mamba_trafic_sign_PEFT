...
class SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
        ...
        # `selective_scan_cuda_oflex.fwd` 的簽名為：
        # fwd(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1, bidirectional=False)
        # 這裡對應地傳入目前 context 中的 `delta_bias`、`delta_softplus` 以及 `nrows`
        out, x, *rest = selective_scan_cuda.fwd(
            u, delta, A, B, C, D, delta_bias, delta_softplus, ctx.nrows, False
        )
